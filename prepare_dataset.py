...
def copy_file(src_path, dest_path):
    """
    Copy file directly without conversion
    """
...
def prepare_subset(images, subset_name, type_name, count):
    """
    Take count images from list and copy to target folder
    """
    ...
    for img_path in tqdm(chosen, desc=f"{subset_name}/{type_name}"):
        dest_file = subset_dir / img_path.name  # Keep original filename and extension
        copy_file(img_path, dest_file)
...
def main():
    ...
    if len(cover_images) < total_needed or len(stego_images) < total_needed:
        print(f"⚠️  Warning: Not enough images!")
        print(f"   Needed: {total_needed} per type")
        print(f"   Found: {len(cover_images)} cover, {len(stego_images)} stego")
        return
