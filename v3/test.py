...
def test_model(args):
    ...
    print("="*70)
    print(f"Cover test path: {args.cover_test_path}")
    print(f"Stego test path: {args.stego_test_path}")

    test_loader = get_test_loader(
        args.cover_test_path,
        args.stego_test_path,
        args.batch_size,
        args.img_size,
        args.num_workers
    )
    ...
    plt.hist(all_probs[all_labels == 0], bins=50, alpha=0.6,
             label='Cover (Class 0)', color='blue', edgecolor='black')
    plt.hist(all_probs[all_labels == 1], bins=50, alpha=0.6,
             label='Stego (Class 1)', color='red', edgecolor='black')
    ...
    # Save plot
    output_path = os.path.join(args.output_dir, 'test_results_steganalysis.png')
    ...
    results_path = os.path.join(args.output_dir, 'test_results_steganalysis.txt')
    with open(results_path, 'w') as f:
        f.write("="*70 + "\n")
        f.write("Steganalysis Classification Test Results\n")
        f.write("="*70 + "\n\n")
        f.write(f"Model: {args.model}\n")
        f.write(f"Model path: {args.model_path}\n")
        f.write(f"Cover test path: {args.cover_test_path}\n")
        f.write(f"Stego test path: {args.stego_test_path}\n\n")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Test Steganalysis Classifier')

    # Data paths - BOWS2 WOW 0.2bpp
    parser.add_argument(
        "--cover_test_path",
        type=str,
        default="/Users/dmitryhoma/Projects/datasets/ready_to_use/GBRASNET/BOWS2/cover/test",
        help="Path to test cover images"
    )
    parser.add_argument(
        "--stego_test_path",
        type=str,
        default="/Users/dmitryhoma/Projects/datasets/ready_to_use/GBRASNET/BOWS2/stego/WOW/0.2bpp/test",
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./test_results_steganalysis",
